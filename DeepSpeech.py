...
def get_tower_results(model_feeder, optimizer):
    ...
    * The mean edit distance of the outcome averaged over the whole batch ``mean_edit_distance``
    ...
    avg_loss = tf.reduce_mean(tower_avg_losses, 0)

    tf.summary.scalar(name='step_loss', tensor=avg_loss, collections=['step_summaries'])
...
def train(server=None):
    ...
    step_summary_writers = {
        'train': tf.summary.FileWriter(os.path.join(FLAGS.summary_dir, 'train')),
        'dev': tf.summary.FileWriter(os.path.join(FLAGS.summary_dir, 'dev')),
        'test': tf.summary.FileWriter(os.path.join(FLAGS.summary_dir, 'test'))
    }
